...
%if you have to do all the point picking set to TRUE and pick a file name
%if the point picking has been done and is saved set to FALSE
doManualAdjust = false;
dataFileName = 'NotFiltered_ima1_2';
...
% if you aren't doing a manual adjust load the data corresponding to the
% selected file
if ~doManualAdjust
    [dataFileName dataFilePath] = uigetfile(fullfile(pwd,'*.mat'),'Select a file','MultiSelect','off');
